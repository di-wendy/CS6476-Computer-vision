...
def main():
    ...
    dmax = 8  # TODO: define the max disparity (int)
    ...
    w_size = 9# TODO: define the window size (int)
    ...
    sigma = 0.1  # TODO: choose a value for sigma
    ...
    L1 = cv2.normalize(image_L , alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    R1 = cv2.normalize(image_R, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    
    cv2.imwrite(os.path.join(output_dir, 'L.png'), L1)
    cv2.imwrite(os.path.join(output_dir, 'LR.png'), R1)
